...
export default function MedicalAttentions() {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'pending' | 'calculated' | 'paid'>('all');
  ...
  // Queries with improved filtering
  const { data: allAttentions = [], isLoading } = useQuery({
    queryKey: ['/api/medical-attentions', activeTab, dateFrom, dateTo],
    queryFn: async () => {
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                <div className="flex space-x-2">
                  ...
                  <Button
                    variant={activeTab === 'calculated' ? 'default' : 'outline'}
                    size="sm"
            ...
            <div className="flex items-center justify-between pt-2 border-t">
              <div className="text-sm text-muted-foreground">
                ...
                {activeTab === 'all' ? ' totales' : ` ${activeTab === 'pending' ? 'pendientes' : activeTab === 'calculated' ? 'calculadas' : 'pagadas'}`}
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                {activeTab === 'pending' ? "No hay atenciones pendientes" : activeTab === 'calculated' ? "No hay atenciones calculadas" : activeTab === 'paid' ? "No hay atenciones pagadas" : "No hay atenciones médicas registradas"}
              </h3>
              <p className="text-gray-500 text-center mb-4">
                {activeTab === 'pending' 
                  ? "Todas las atenciones han sido calculadas o pagadas"
                  : activeTab === 'calculated'
                  ? "No hay atenciones con estado calculado en este período"
                  : activeTab === 'paid'
                  ? "No hay atenciones pagadas en este período"
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <p className="text-sm font-medium">ID Excel: {attention.externalId}</p>
                        <p className="text-sm text-gray-600">
                          Bruto: ${parseFloat(attention.grossAmount).toLocaleString('es-CL')}
                        </p>
                        <p className="text-xs text-gray-500">
                          Neto: ${parseFloat(attention.netAmount).toLocaleString('es-CL')} | 
                          Participado: ${parseFloat(attention.participatedAmount).toLocaleString('es-CL')}
                        </p>
